...
namespace Microsoft.AspNetCore.Components
{
    ...
    public abstract partial class NavigationManager
    {
        ...
        private readonly List<Func<LocationChangingContext, Task>> _locationChangingHandlers = new List<Func<LocationChangingContext, Task>>();
        ...
        internal ReadOnlySpan<char> ToBaseRelativePath(ReadOnlySpan<char> uri)
        {
            if (uri.StartsWith(_baseUri.OriginalString().AsSpan(), StringComparison.Ordinal))
            {
                // The absolute URI must be of the form "{baseUri}something" (where
                // baseUri ends with a slash), and from that we return "something"
                return uri[new System.Range(_baseUri.OriginalString().Length, -1)];
            }

            var pathEndIndex = uri.IndexOfAny('#', '?');
            var uriPathOnly = pathEndIndex < 0 ? uri : uri[new System.Range(0, pathEndIndex)];
            if (_baseUri.OriginalString().EndsWith("/") && uriPathOnly.IsEqual(_baseUri.OriginalString().AsSpan(0, _baseUri.OriginalString().Length - 1), StringComparison.Ordinal))
            {
                ...
                // slash is present, but ASP.NET Core at least does by default when
                // using PathBase.
                return uri[new System.Range((_baseUri.OriginalString().Length - 1), -1)];
            }
        ...
        /// <returns>A <see cref="Task{TResult}"/> representing the completion of the operation. If the result is <see langword="true"/>, the navigation should continue.</returns>
        protected async Task<bool> NotifyLocationChangingAsync(string uri, object state, bool isNavigationIntercepted)
        {
            ...
            try
            {
                ...
                {
                    var locationChangingHandlersCopy = ArrayPool<Func<LocationChangingContext, Task>>.Shared.Rent(handlerCount);
        ...
        private async Task InvokeLocationChangingHandlerAsync(Func<LocationChangingContext, Task> handler, LocationChangingContext context)
        {
            try
            {
        ...
        public IDisposable RegisterLocationChangingHandler(Func<LocationChangingContext, Task> locationChangingHandler)
        {
            AssertInitialized();
        ...
        private void RemoveLocationChangingHandler(Func<LocationChangingContext, Task> locationChangingHandler)
        {
            AssertInitialized();
        ...
        private sealed partial class LocationChangingRegistration : IDisposable
        {
            private readonly Func<LocationChangingContext, Task> _handler;
            private readonly NavigationManager _navigationManager;

            public LocationChangingRegistration(Func<LocationChangingContext, Task> handler, NavigationManager navigationManager)
            {
                _handler = handler;
