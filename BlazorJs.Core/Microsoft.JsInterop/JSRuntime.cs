...
namespace Microsoft.JSInterop
{
    ...
    public abstract partial class JSRuntime : IJSRuntime, IDisposable
    {
        ...
        internal readonly List<byte[]> ByteArraysToBeRevived = new List<byte[]>();
        ...
        public async Task SetValueAsync<TValue>(string identifier, TValue value)
            => await InvokeAsync<IJSVoidResult>(WindowObjectId, identifier, JSCallType.SetValue, new object[] { value });

        /// <inheritdoc />
        public async Task SetValueAsync<TValue>(string identifier, TValue value, CancellationToken cancellationToken)
            => await InvokeAsync<IJSVoidResult>(WindowObjectId, identifier, JSCallType.SetValue, cancellationToken, new object[] { value });

        internal async Task<TValue> InvokeAsync<TValue>(long targetInstanceId, string identifier, JSCallType callType, object[] args)
        {
            if (DefaultAsyncTimeout.HasValue)
            {
                var cts = new CancellationTokenSource(DefaultAsyncTimeout.Value);
                try
                {
        ...
        internal Task<TValue> InvokeAsync<TValue>(
            ...
            object[] args)
        {
            var taskId = _nextPendingTaskId++;// Interlocked.Increment(ref _nextPendingTaskId);
            ...
            try
            {
                if (cancellationToken.IsCancellationRequested)
                {
                    ...
                    return tcs.Task;// new Task<TValue>(tcs.Task);
                }
        ...
        internal bool EndInvokeJS(long taskId, bool succeeded, object jsResult/* ref Utf8JsonReader jsonReader*/)
        {
            ...
            try
            {
                if (succeeded)
                {
                    ...
                    var result = JsonSerializer.Convert(jsResult, resultType);
                    ByteArraysToBeRevived.Clear();
                    TaskGenericsUtil.SetTaskCompletionSourceResult(tcs, result);
                }
                else
                {
                    var exceptionText = jsResult.As<string>() ?? string.Empty;
        ...
        internal long BeginTransmittingStream(DotNetStreamReference dotNetStreamReference)
        {
            ...
            var streamId = _nextObjectReferenceId++;// Interlocked.Increment(ref _nextObjectReferenceId);
        ...
        internal long TrackObjectReference<TValue>(DotNetObjectReference<TValue> dotNetObjectReference) where TValue : class
        {
            ...
            {
                var dotNetObjectId = _nextObjectReferenceId++;// Interlocked.Increment(ref _nextObjectReferenceId);
        ...
        /// </summary>
        public void Dispose() => ByteArraysToBeRevived.Clear();//.Dispose();
    }
}