...
namespace Microsoft.JSInterop.Infrastructure
{
    ...
    public static class DotNetDispatcher
    {
        ...
        public static void BeginInvokeDotNet(JSRuntime jsRuntime, DotNetInvocationInfo invocationInfo, string argsJson)
        {
            ...
            {
                syncException = ex;// ExceptionDispatchInfo.Capture(ex);
            }
            ...
            else if (syncResult is Task task)
            {
                ...
                // or return the value.
                task.ContinueWith(t => EndInvokeDotNetAfterTask(t, jsRuntime, invocationInfo));

            }
            else if (syncResult is Task TaskResult)
            {
                TaskResult.ContinueWith(t => EndInvokeDotNetAfterTask(t, jsRuntime, invocationInfo));
            }
            else if (syncResult != null && syncResult.GetType().IsGenericType
                && syncResult.GetType().GetGenericTypeDefinition() == typeof(Task<>))
            {
                // It's a Task<T>. We'll coerce it to a Task so that we can attach a continuation.
                var innerTask = GetTaskByType(syncResult?.GetType().GetGenericArguments()[0]/*.GenericTypeArguments[0]*/, syncResult);

                innerTask.ContinueWith(t => EndInvokeDotNetAfterTask(t, jsRuntime, invocationInfo));
            }
            else
            {
        ...
        private static void EndInvokeDotNetAfterTask(Task task, JSRuntime jsRuntime, in DotNetInvocationInfo invocationInfo)
        {
            ...
            {
                var exceptionDispatchInfo = task.Exception.GetBaseException();//ExceptionDispatchInfo.Capture(task.Exception.GetBaseException());
        ...
        internal static object[] ParseArguments(JSRuntime jsRuntime, string methodIdentifier, string arguments, Type[] parameterTypes)
        {
            ...
            try
            {
                var datas = JsonSerializer.DeserializeImpl(arguments).As<object[]>();
                ...
                //var buffer = Encoding.UTF8.GetBytes(arguments);
        ...
        {
            var data = JsonSerializer.DeserializeImpl(arguments).As<object[]>();
            var taskId = data[0].As<long>();
            var success = data[1].As<bool>();
        ...
        private static Task GetTaskByType(Type type, object obj)
        {
            var converterDelegate = _cachedConvertToTaskByType.GetOrAdd(type, (Type t, MethodInfo taskConverterMethodInfo) =>
                (Func<object, Task>)taskConverterMethodInfo.MakeGenericMethod(t).CreateDelegate/*<Func<object, Task>>*/(), _taskConverterMethodInfo);

            return converterDelegate.Invoke(obj);
        }
        private static Task CreateValueTaskConverter<T>(object result) => (Task<T>)result;
...
#pragma warning restore IL2111 // Method with parameters or return value with `DynamicallyAccessedMembersAttribute` is accessed via reflection. Trimmer can't guarantee availability of the requirements of the method.
            ...
            Dictionary<string, (MethodInfo, Type[])> ScanTypeForCallableMethods(Type itype)
            {
                var iresult = new Dictionary<string, (MethodInfo, Type[])>(StringComparer.Ordinal);

                foreach (var method in itype.GetMethods(BindingFlags.Instance | BindingFlags.Public))
                {
                    ...
                    foreach (var attr in method.GetCustomAttributes<JSInvokableAttribute>(false))
                    {
                        ...
                        {
                            throw new InvalidOperationException($"The type {itype.Name} contains more than one " +
                        ...
                        iresult.Add(identifier, (method, parameterTypes));
                    }
                }
        ...
        private static Dictionary<string, (MethodInfo, Type[])> ScanAssemblyForCallableMethods(AssemblyKey assemblyKey)
        {
            ...
            var exportedTypes = GetRequiredLoadedAssembly(assemblyKey).GetTypes();// GetExportedTypes();
        ...
        private static Type[] GetParameterTypes(MethodInfo method)
        {
            ...
            {
                return Array.Empty<Type>();// Type.EmptyTypes;
            }
