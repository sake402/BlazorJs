...
namespace System.Net.Http
{
    public abstract partial class HttpContent : IDisposable
    {
        ...
        private static ReadOnlySpan<byte> UTF32Preamble => new byte[] { 0xFF, 0xFE, 0x00, 0x00 };
        private static ReadOnlySpan<byte> UnicodePreamble => new byte[] { 0xFF, 0xFE };
        private static ReadOnlySpan<byte> BigEndianUnicodePreamble => new byte[] { 0xFE, 0xFF };
        ...
        private bool IsBuffered => !(_bufferedContent is null);
        ...
        internal static string ReadBufferAsString(LimitArrayPoolWriteStream stream, HttpContentHeaders headers)
        {
            ...
            else
            {
                ...
                string result = encoding.GetString(buffer, bomLength, buffer.Length - bomLength);
        ...
        protected virtual Task SerializeToStreamAsync(Stream stream, CancellationToken cancellationToken) =>
        ...
        static async Task WaitAsync(Task copyTask)
        {
            try
            {
                await copyTask;
        ...
        internal Task InternalCopyToAsync(Stream stream, CancellationToken cancellationToken)
        {
            if (IsBuffered)
            {
            ...
            Task task = SerializeToStreamAsync(stream, cancellationToken);
        ...
        public Task LoadIntoBufferAsync(long maxBufferSize, CancellationToken cancellationToken)
        {
            ...
            try
            {
                Task task = SerializeToStreamAsync(tempBuffer, cancellationToken);
                CheckTaskNotNull(task);
        ...
        protected virtual Task<Stream> CreateContentReadStreamAsync()
        {
            ...
            return WaitAndReturnAsync(LoadIntoBufferAsync(), this, s => (Stream)s.CreateMemoryStreamFromBufferedContent());
        }
        ...
        private void CheckTaskNotNull(Task task)
        {
            if (task == null)
            {
                var e = new InvalidOperationException("net_http_content_no_task_returned");
                throw e;
            }
        }
        ...
        private static int GetPreambleLength(ReadOnlySpan<byte> data, Encoding encoding)
        {
            ...
            switch (encoding.CodePage)
            {
                ...
                default:
                    ...
                    //return !(preamble is null) && data.StartsWith(preamble) ? preamble.Length : 0;
                    return 0;
            }
        }

        private static bool TryDetectEncoding(ReadOnlySpan<byte> data, out Encoding encoding, out int preambleLength)
        {
        ...
        /// </summary>
        internal sealed partial class LimitArrayPoolWriteStream : AsyncStream
        {
...
//#endif
            ...
            public ReadOnlySpan<byte> GetFirstBuffer()
            {
                return _pooledBuffers is byte[][] buffers ?
                     buffers[0] : _lastBuffer.AsReadOnlySpan(0, _totalLength);
            }
            ...
            private void GrowAndWrite(ReadOnlySpan<byte> buffer)
            {
                ...
                int newBufferCapacity = (int)Math.Min((uint)lastBufferCapacity * ResizeFactor, 100 * 1024 * 1024);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        while (bufferCount < buffers.Length && !(buffers[bufferCount] is null))
                        {
                            bufferCount++;
                        }
            ...
            public void CopyToCore(Span<byte> destination)
            {
                ...
                {
                    Debug.Assert(buffers.Length > 0 && !(buffers[0] is null));
            ...
            private void ReturnAllPooledBuffers()
            {
                ...
                Debug.Assert(!(_lastBuffer is null));
                byte[] lastBuffer = _lastBuffer;
                _lastBuffer = null;
