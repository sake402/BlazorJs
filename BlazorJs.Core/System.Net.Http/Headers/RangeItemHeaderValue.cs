...
{
    public partial class RangeItemHeaderValue : ICloneable
    {
        // Set to -1 if not set.
        private readonly long _from;
        private readonly long _to;

        public long? From => _from >= 0 ? _from : (long?)null;

        public long? To => _to >= 0 ? _to : (long?)null;
        ...
        {
            Span<char> stackBuffer = new Span<char>(128);
        ...
        internal static int GetRangeItemLength(string input, int startIndex, out RangeItemHeaderValue parsedValue)
        {
            ...
            parsedValue = new RangeItemHeaderValue((fromLength == 0 ? (long?)null : from), (toLength == 0 ? (long?)null : to));
            return current - startIndex;
        }
