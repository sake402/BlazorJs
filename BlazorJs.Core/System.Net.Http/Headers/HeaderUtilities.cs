...
namespace System.Net.Http.Headers
{
    internal static partial class HeaderUtilities
    {
        ...
        //      ; token except ( "*" / "'" / "%" )
        private static readonly string s_rfc5987AttrBytes = "!#$&+-.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ^_`abcdefghijklmnopqrstuvwxyz|~";
        ...
        internal static string Encode5987(string input)
        {
            ...
            ReadOnlySpan<byte> utf8 = utf8bytes.AsReadOnlySpan(0, utf8length);
            do
            {
                int length = utf8.IndexOfAnyExcept(Encoding.ASCII.GetBytes(s_rfc5987AttrBytes));
                if (length < 0)
                {
                    length = utf8.Length;
                }

                foreach(var b in utf8.Slice(0, length))
                {
                    builder.Append(b);
                }
        ...
        private static void AddHexEscaped(byte c, ref StringBuilder destination)
        {
            destination.Append('%');
        ...
        internal static double? GetQuality(UnvalidatedObjectCollection<NameValueHeaderValue> parameters)
        {
            ...
            if (qualityParameter != null)
            {
                ...
                double qualityValue;
                if (double.TryParse(qualityParameter.Value, out qualityValue))
                {
                    return qualityValue;
                }
        ...
        internal static bool TryParseInt32(string value, int offset, int length, out int result)
        {
            ...
            return int.TryParse(value.AsSpan(offset, length), out result);
        }
        ...
        internal static bool TryParseInt64(string value, int offset, int length, out long result)
        {
            ...
            return long.TryParse(value.AsSpan(offset, length), out result);
        }
