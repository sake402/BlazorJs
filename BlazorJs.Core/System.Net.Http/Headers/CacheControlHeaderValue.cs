...
{
    public partial class CacheControlHeaderValue : ICloneable
    {
        ...
        {
            get => (_flags & Flags.MaxAgeHasValue) == 0 ? (TimeSpan?)null : _maxAge;
            set => SetTimeSpan(ref _maxAge, Flags.MaxAgeHasValue, value);
        }

        public TimeSpan? SharedMaxAge
        {
            get => (_flags & Flags.SharedMaxAgeHasValue) == 0 ? (TimeSpan?)null : _sharedMaxAge;
        ...
        {
            get => (_flags & Flags.MaxStaleLimitHasValue) == 0 ? (TimeSpan?)null : _maxStaleLimit;
            set => SetTimeSpan(ref _maxStaleLimit, Flags.MaxStaleLimitHasValue, value);
        }

        public TimeSpan? MinFresh
        {
            get => (_flags & Flags.MinFreshHasValue) == 0 ? (TimeSpan?)null : _minFresh;
            ...
            out CacheControlHeaderValue parsedValue)
        {
            ...
            while (current < input.Length)
            {
                ...
                Debug.Assert(!(nameValue is null));
        ...
        private static bool TrySetCacheControlValues(CacheControlHeaderValue cc, List<NameValueHeaderValue> nameValueList)
        {
            ...
            {
                string name = nameValue.Name.ToLower();
        ...
        private sealed partial class TokenObjectCollection : ObjectCollection<string>
        {
