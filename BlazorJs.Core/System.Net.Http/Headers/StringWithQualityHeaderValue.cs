...
{
    public partial class StringWithQualityHeaderValue : ICloneable
    {
        ...
        public double? Quality => _quality == NotSetSentinel ? (double?)null : _quality;
        ...
        private static bool TryReadQuality(string input, out double quality, ref int index)
        {
            ...
            if (!double.TryParse(input.AsSpan(current, qualityLength), out quality))
            {
                return false;
            }
