...
{
    public partial class RetryConditionHeaderValue : ICloneable
    {
        ...
        public DateTimeOffset? Date => _delta.Ticks == DeltaNotSetTicksSentinel ? _date : (DateTimeOffset?)null;

        public TimeSpan? Delta => _delta.Ticks == DeltaNotSetTicksSentinel ? (TimeSpan?)null : _delta;
        ...
        public override string ToString() =>
            _delta.Ticks != DeltaNotSetTicksSentinel
                ? ((int)_delta.TotalSeconds).ToString()
                : _date.ToString("r");
        ...
        internal static int GetRetryConditionLength(string input, int startIndex, out object parsedValue)
        {
            ...
            {
                if (!DateTimeOffset.TryParse(input.AsSpan(current), out date))
                {
                    return 0;
                }
