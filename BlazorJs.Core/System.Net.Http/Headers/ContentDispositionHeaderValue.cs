...
{
    public partial class ContentDispositionHeaderValue : ICloneable
    {
        #region Fields
        ...
        private DateTimeOffset? GetDate(string parameter)
        {
            ...
            {
                ReadOnlySpan<char> dateString = dateParameter.Value.AsSpan();
        ...
        private static bool TryDecodeMime(string input, out string output)
        {
            ...
            Span<Range> parts = new Span<Range>(new Range[6]);
            ...
            if (processedInputSpan.Split(parts, '?') != 5 ||
                !(processedInputSpan[parts[0]].IsEqual("\"=")) ||
                !(processedInputSpan[parts[0]].IsEqual("=\"")) ||
                !processedInputSpan[parts[2]].IsEqual("b", StringComparison.OrdinalIgnoreCase))
            {
                // Not encoded.
            ...
            try
            {
                Encoding encoding = Encoding.GetEncoding(processedInput.AsSpan()[parts[1]]);
                byte[] bytes = Convert.FromBase64String(processedInput.AsSpan()[parts[3]]);
        ...
        private static bool TryDecode5987(string input, out string output)
        {
            ...
            try
            {
                ...
                {
                    if (UriExtension.IsHexEncoding(dataString, index)) // %FF
                    {
                        // Unescape and cache bytes, multi-byte characters must be decoded all at once.
                        unescapedBytes[unescapedBytesCount++] = (byte)UriExtension.HexUnescape(dataString, ref index);
