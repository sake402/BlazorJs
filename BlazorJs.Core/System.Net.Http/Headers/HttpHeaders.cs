...
namespace System.Net.Http.Headers
{
    ...
    public abstract partial class HttpHeaders : IEnumerable<KeyValuePair<string, IEnumerable<string>>>
    {
        ...
        internal bool TryAddWithoutValidation(HeaderDescriptor descriptor, string value)
        {
            ...
            var storeValueRef = GetValueRefOrAddDefault(descriptor);
            object currentValue = storeValueRef.Value;
        ...
        internal bool TryAddWithoutValidation(HeaderDescriptor descriptor, IEnumerable<string> values)
        {
            ...
            if (values is IList<string> valuesList)
            {
                ...
                if (count > 0)
                {
                    ...
                    var storeValueRef = GetValueRefOrAddDefault(descriptor);
                    object storeValue = storeValueRef.Value;
                    ...
                    if (!(storeValue is null) || count > 1)
                    {
                        if (!(storeValue is HeaderStoreItemInfo info))
                        {
                            storeValueRef.Value = info = new HeaderStoreItemInfo { RawValue = storeValue };
                        }

                        object rawValue = info.RawValue;
                        if (!(rawValue is List<string> rawValues))
                        {
        ...
        public override string ToString()
        {
            ...
            foreach (HeaderEntry entry in GetEntries())
            {
                ...
                else
                {
                    ...
                    Debug.Assert(!(multiValue is null) && multiValue.Length > 0);
        ...
        {
            if (TryGetHeaderValue(descriptor, out var info))
            {
        ...
        private IEnumerator<KeyValuePair<string, IEnumerable<string>>> GetEnumeratorCore()
        {
            ...
            Debug.Assert(_count != 0 && !(entries is null), "Caller should have validated the collection is not empty");
            ...
            for (int i = 0; i < _count; i++)
            {
                ...
                if (!(entry.Value is HeaderStoreItemInfo info))
                {
                    ...
                    var storeValueRef = EntriesAreLiveView ?
                          entries[i] :
                          ((Dictionary<HeaderDescriptor, HeaderEntry>)_headerStore).GetValueOrDefault(entry.Key);
        ...
        internal virtual void AddHeaders(HttpHeaders sourceHeaders)
        {
            ...
            if (_count == 0 && sourceHeaders._headerStore is HeaderEntry[] sourceEntries)
            {
                ...
                if (!(_headerStore is HeaderEntry[] entries) || entries.Length < _count)
                {
            ...
            else
            {
                ...
                {
                    var storeValueRef = GetValueRefOrAddDefault(entry.Key);
        ...
        internal bool TryGetHeaderValue(HeaderDescriptor descriptor, out HeaderEntry value)
        {
            //ref object storeValueRef = ref GetValueRefOrNullRef(descriptor);
            var storeValueRef = GetValueRefOrNullRef(descriptor);
            if (storeValueRef == null/*Unsafe.IsNullRef(ref storeValueRef)*/)
            {
                value = null;
                return false;
            }
        ...
        {
            var storeValueRef = GetValueRefOrNullRef(key);
            if (!(storeValueRef == null)/*!Unsafe.IsNullRef(ref storeValueRef)*/)
            {
                object value = storeValueRef.Value;

                info = value is HeaderStoreItemInfo hsi ?
                     hsi
                    : ReplaceWithHeaderStoreItemInfo(storeValueRef, value);
        ...
        private static HeaderStoreItemInfo ReplaceWithHeaderStoreItemInfo(HeaderEntry storeValueRef, object value)
        {
            Debug.Assert(value is string);

            var info = new HeaderStoreItemInfo() { RawValue = value };

            object previousValue = InterlockedExtension.CompareExchange(ref storeValueRef.Value, info, value);
        ...
        internal static void GetStoreValuesAsStringOrStringArray(HeaderDescriptor descriptor, object sourceValues, out string singleValue, out string[] multiValue)
        {
            ...
            if (info is null)
            {
                HeaderEntry entry = (HeaderEntry)sourceValues;
                Debug.Assert(entry.Value is string);
                singleValue = (string)entry.Value;
                multiValue = null;
                return;
            }
            ...
            lock (info)
            {
                ...
                {
                    multiValue = null;
                    values = new Span<string>(new[] { singleValue });
                }
                else
                {
        ...
        internal static int GetStoreValuesIntoStringArray(HeaderDescriptor descriptor, object sourceValues, ref string[] values)
        {
        ...
        static  int CountObj<T>(object valueStore) =>
        ...
        private static int GetValueCount(HeaderStoreItemInfo info)
        {
            ...
            return CountObj<object>(info.ParsedAndInvalidValues) + CountObj<string>(info.RawValue);
        }
        ...
        private HeaderEntry GetValueRefOrNullRef(HeaderDescriptor key)
        {
            HeaderEntry valueRef = null;//ref Unsafe.NullRef<object>();
            ...
            {
                valueRef = ((Dictionary<HeaderDescriptor, HeaderEntry>)store).GetValueOrDefault(key);
        ...
        private HeaderEntry GetValueRefOrAddDefault(HeaderDescriptor key)
        {
            object store = _headerStore;
            if (store is HeaderEntry[] entries)
            {
            ...
            else if (store is null)
            {
                ...
                _headerStore = entries;
                HeaderEntry firstEntry = entries[0];// ref MemoryMarshal.GetArrayDataReference(entries);
                firstEntry.Key = key;
                return firstEntry;
            }
            else
            ...
            HeaderEntry GrowEntriesAndAddDefault(HeaderDescriptor mkey)
            {
                var mentries = (HeaderEntry[])_headerStore;
                ...
                {
                    Array.Resize(ref mentries, mentries.Length << 1);
                    _headerStore = mentries;
                    HeaderEntry firstNewEntry = mentries[mentries.Length >> 1];
                    firstNewEntry.Key = mkey;
                    return firstNewEntry;
                }
            }

            HeaderEntry ConvertToDictionaryAndAddDefault(HeaderDescriptor mkey)
            {
                var mentries = (HeaderEntry[])_headerStore;
                var dictionary = new Dictionary<HeaderDescriptor, HeaderEntry>(ArrayThreshold);
                _headerStore = dictionary;
                foreach (HeaderEntry entry in mentries)
                {
                    dictionary.Add(entry.Key, entry);
                }
                Debug.Assert(dictionary.Count == _count - 1);
                if (!dictionary.TryGetValue(mkey, out var value))
                {
                    value = new HeaderEntry { Key = mkey };
                    dictionary[mkey] = value;
                }
                return value;
                //return ref CollectionsMarshal.GetValueRefOrAddDefault(dictionary, mkey, out _);
            }

            HeaderEntry DictionaryGetValueRefOrAddDefault(HeaderDescriptor mkey)
            {
                var dictionary = (Dictionary<HeaderDescriptor, HeaderEntry>)_headerStore;
                //ref CollectionsMarshal.GetValueRefOrAddDefault(dictionary, mkey, out _);
                if (!dictionary.TryGetValue(mkey, out var value))
                {
                    value = new HeaderEntry { Key = mkey };
                    dictionary[mkey] = value;
        ...
        private void AddEntryToStore(HeaderEntry entry)
        {
            ...
            var val = GetValueRefOrAddDefault(entry.Key);
            if (val != null)
        ...
        internal bool Remove(HeaderDescriptor key)
        {
            ...
            {
                removed = ((Dictionary<HeaderDescriptor, HeaderEntry>)store).Remove(key);
            }
