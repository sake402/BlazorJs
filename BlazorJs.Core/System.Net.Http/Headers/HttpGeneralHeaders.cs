...
namespace System.Net.Http.Headers
{
    ...
    internal sealed partial class HttpGeneralHeaders
    {
        ...
        public HttpHeaderValueCollection<NameValueHeaderValue> Pragma =>
            _pragma = _pragma ?? new HttpHeaderValueCollection<NameValueHeaderValue>(KnownHeaders.Pragma.Descriptor, _parent);

        public HttpHeaderValueCollection<string> Trailer =>
            _trailer = _trailer ?? new HttpHeaderValueCollection<string>(KnownHeaders.Trailer.Descriptor, _parent);

        internal static bool? GetTransferEncodingChunked(HttpHeaders parent, HttpGeneralHeaders headers)
        {
            if (parent.TryGetHeaderValue(KnownHeaders.TransferEncoding.Descriptor, out HeaderEntry value))
            {
                // Fast-path for the very common case where "chunked" is the only value.
                if (value.Value is string stringValue && stringValue.Equals("chunked", StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
        ...
        public HttpHeaderValueCollection<ProductHeaderValue> Upgrade =>
            _upgrade = _upgrade ?? new HttpHeaderValueCollection<ProductHeaderValue>(KnownHeaders.Upgrade.Descriptor, _parent);

        public HttpHeaderValueCollection<ViaHeaderValue> Via =>
            _via = _via ?? new HttpHeaderValueCollection<ViaHeaderValue>(KnownHeaders.Via.Descriptor, _parent);

        public HttpHeaderValueCollection<WarningHeaderValue> Warning =>
            _warning = _warning ?? new HttpHeaderValueCollection<WarningHeaderValue>(KnownHeaders.Warning.Descriptor, _parent);

        public HttpHeaderValueCollection<string> Connection =>
            _connection = _connection ?? new HttpHeaderValueCollection<string>(KnownHeaders.Connection.Descriptor, _parent);

        public HttpHeaderValueCollection<TransferCodingHeaderValue> TransferEncoding =>
            _transferEncoding = _transferEncoding ?? new HttpHeaderValueCollection<TransferCodingHeaderValue>(KnownHeaders.TransferEncoding.Descriptor, _parent);
