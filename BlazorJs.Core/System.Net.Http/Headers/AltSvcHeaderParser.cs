...
namespace System.Net.Http.Headers
{
    ...
    /// </summary>
    internal sealed partial class AltSvcHeaderParser : BaseHeaderParser
    {
        ...
        private static bool TryReadPercentEncodedAlpnProtocolName(string value, int startIndex, out string result, out int readLength)
        {
            ...
            switch (span.Length)
            {
                case 2 when span.IsEqual("h3"):
                    result = "h3";
                    return true;

                case 2 when span.IsEqual("h2"):
                    result = "h2";
                    return true;

                case 3 when span.IsEqual("h2c"):
                    result = "h2c";
                    readLength = 3;
                    return true;

                case 5 when span.IsEqual("clear"):
                    result = "clear";
                    return true;
        ...
        {
            int result = ch >= '0' && ch < '9' ? ch - '0' : ch >= 'A' && ch <= 'F' ? (ch - 'A' + 10) : 0xFF;// HexConverter.FromUpperChar(ch);
            if (result == 0xFF)
            {
                nibble = 0;
                return false;
