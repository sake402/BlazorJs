...
namespace System.Net.Http.Headers
{
    ...
    internal sealed partial class UriHeaderParser : HttpHeaderParser
    {
        ...
        public override bool TryParseValue(string value, object storeValue, ref int index, out object parsedValue)
        {
            ...
            if (!UriExtension.TryCreate(uriString, _uriKind, out Uri uri))
            {
                // Some servers send the host names in Utf-8.
                uriString = DecodeUtf8FromString(uriString);

                if (!UriExtension.TryCreate(uriString, _uriKind, out uri))
                {
                    return false;
                }
            }
        ...
        internal static string DecodeUtf8FromString(string input)
        {
            if (!string.IsNullOrWhiteSpace(input))
            {
                ...
                {
                    Span<byte> rawBytes = new byte[input.Length];
